...
function placeXOrO(squareNumber) { //This function is for placing an X or O in a square.
    //The condition below ensures a square hasn't been selected already. The .some() method is used to check each element of selectedSquare array to see if it contains the square number clicked on.
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        if (activePlayer === 'X') { //This condition checks who's turn it is.
            select.style.backgroundImage= 'url("images/x1.jpg")'; //If activePlayer is equal to "X", the x.png is placed in HTML.
        } else { //Active player may only be "X" or "O" so,  if not "X" it must be "O".
            select.style.backgroundImage= 'url("images/o1.png")'; //If activePlayer is equal to "O", the o.png is placed in HTML.
        }
        ...
        audio('./media/place1.mp3'); //This function plays placement sound.
    ...
    function computersTurn() { //This function results in a random square being selected.
        ...
        while(!success) { //This condition allows our while loop to keep trying if a square is selected already.
            ...
            if (placeXOrO(pickASquare)) { //This line calls the function.
                placeXOrO(pickASquare); //This line changes our boolean and ends the loop.
                success= true;
            };
        }
    }
...
function checkWinConditions() { //This function parses the selectedSquares array to search for win conditions. drawWinLine function is called to draw line if condition is met.
    if      (arrayIncludes('0X', '1X', '2X')) { drawWinLine( 50, 100, 558, 100) } // X 0, 1, 2, condition.
    ...
    else if (arrayIncludes('0X', '1X', '2X')) { drawWinLine( 50, 100, 558, 100) } // O 0, 1, 2, condition.
    else if (arrayIncludes('3X', '4X', '5X')) { drawWinLine( 50, 304, 558, 304) } // O 3, 4, 5, condition.
    else if (arrayIncludes('6X', '7X', '8X')) { drawWinLine( 50, 508, 558, 508) } // O 6, 7, 8, condition.
    else if (arrayIncludes('0X', '3X', '6X')) { drawWinLine( 100, 50, 100, 558) } // O 0, 3, 6, condition.
    else if (arrayIncludes('1X', '4X', '7X')) { drawWinLine( 304, 50, 304, 558) } // O 1, 4, 7, condition.
    else if (arrayIncludes('2X', '5X', '8X')) { drawWinLine( 508, 50, 508, 558) } // O 2, 5, 8, condition.
    else if (arrayIncludes('6X', '4X', '2X')) { drawWinLine( 100, 508, 510, 90) } // O 6, 4, 2, condition.
        ...
        audio('./media/tie1.mp3'); //This function plays the tie game sounds.
    ...
    function arrayIncludes(squareA, squareB, squareC) { //This function checks if an array includes 3 strings. It is used to check for each win condition.
        const a= selectedSquares.includes(squareA) //These 3 const variables will be used to check for 3 in a row.
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) { //This function utilizes html canvas to draw win lines.
    ...
    function animateLineDrawing() { //This function interacts with the canvas.
        ...
        if (x1 <= x2 && y1 <= y2) { //This condition checks if we've reached the endpoint.
            ...
            if (x >= x2 && y >= y2) { cancelAnimationFrame(animationLoop); } //This condition cancels our animation loop if we've reached reached the endpoint.
        }
        if (x1 <= x2 && y1 >= y2) { //This condition is similar to the one above. This is neccessary for the 6, 4, 2 win condition.
            if (x < x2) { x += 10; } //See 'if' above.
            if (y > y2) { y -= 10; } //See 'if' above.
            if (x >= x2 && y <= y2) { cancelAnimationFrame(animationLoop); } //See 'if' above.
        }
    }
    function clear() { //This function clears our canvas after our win line is drawn.
        const animationLoop= requestAnimationFrame(clear); //This line starts our animation logo.
    ...
    audio('./media/winGame1.mp3'); //This line plays the win sounds.
